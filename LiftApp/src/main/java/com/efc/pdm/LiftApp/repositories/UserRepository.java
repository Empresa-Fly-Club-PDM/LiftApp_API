...
public interface UserRepository extends JpaRepository<User,Integer> {
    ...
    @Query("SELECT u FROM User u WHERE u.role = 'USER' AND u.enabledstate = TRUE AND u.enabledstate = true AND u.role <> 'ADMIN' AND (u.email LIKE CONCAT('%',:query, '%') OR u.nombrecompleto LIKE CONCAT('%',:query, '%') OR u.fechanac LIKE CONCAT('%',:query, '%') OR u.genero LIKE CONCAT('%',:query, '%') OR u.height LIKE CONCAT('%',:query, '%') OR u.weight LIKE CONCAT('%',:query, '%')) ORDER BY u.points DESC")
